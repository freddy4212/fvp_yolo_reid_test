...
class VideoServer:
    ...
    def __changeColorSpace(self, frame, color_space):
        if   color_space == self.GRAYSCALE8:
            ...
            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2YUV_I420)
            uv_row_cnt = frame.shape[0] // 3
            uv_plane = np.transpose(frame[frame.shape[0] - uv_row_cnt:, :].reshape(2, -1), [1, 0]).reshape(-1, frame.shape[1])
            frame[frame.shape[0] - uv_row_cnt:, :] = uv_plane
    ...
    def _writeFrame(self, frame):
        ...
        try:
            ...
            decoded_frame = decoded_frame.reshape((self.resolution[1], self.resolution[0], 3))
            bgr_frame = self.__changeColorSpace(decoded_frame, self.RGB888)

            if self.filename == "":
                # logging.info("Displaying frame...")
                # cv2.imshow("LCD Output", bgr_frame)
                ...
                # Use frame_index for naming, ensure it increments
                save_path = os.path.join(output_dir, f"frame_{self.frame_index:04d}.jpg")
                cv2.imwrite(save_path, bgr_frame)
                logging.info(f"Saved frame to {save_path}")
                self.frame_index += 1
            ...
            logging.error(f"Error in _writeFrame: {e}")
            pass
    ...
    def run(self):
        ...
        while True:
            try:
                # Poll for data with a short timeout to keep window responsive
                if conn.poll(0.01):
                    recv = conn.recv()
                else:
                    # If no data, process window events
