    ...
    if ((value ^ CONTROL) & CONTROL_ENABLE_Msk) != 0:
        ...
        if (value & CONTROL_ENABLE_Msk) != 0:
            ...
            if Video.conn != None:
                ...
                logging.error("Server not connected")
                print("Py: VSI1: Server not connected in wrCONTROL")
                sys.stdout.flush()
        else:
...
## Process frame for File I/O (Fake DMA)
def process_frame_file_out():
    global STATUS, FRAME_COUNT
    
    if (STATUS & STATUS_ACTIVE_Msk) != 0:
        # Read from file
        try:
            with open("frame_buffer_out.bin", "rb") as f:
                data = f.read()
            ...
            logging.error(f"Failed to read frame_buffer_out.bin: {e}")
...
def wrFILENAME_LEN(value):
    global FILENAME_LEN, Filename, FilenameIdx
...
def wrFILENAME_CHAR(value):
    global FILENAME_CHAR, FILENAME_VALID, Filename, FilenameIdx
    ...
    if FilenameIdx == FILENAME_LEN:
        logging.info("Set filename: " + Filename)
        if Video.conn != None:
            FILENAME_VALID = Video.setFilename(Filename, MODE)
            if FILENAME_VALID:
                logging.info("Filename valid")
            else:
                logging.error("Filename invalid")
        else:
            logging.error("Server not connected")
