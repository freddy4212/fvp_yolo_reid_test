...
// The following definition have been added to
// IRQn_Type's definition to avoid issues with miss-auto-sized enums.
...
static inline ARM_VSI_Type* arm_vsi_get_base(uint32_t channel) {
    switch(channel) {
        case 0: return ARM_VSI0;
        case 1: return ARM_VSI1;
        case 2: return ARM_VSI2;
        case 3: return ARM_VSI3;
        case 4: return ARM_VSI4;
        case 5: return ARM_VSI5;
        case 6: return ARM_VSI6;
        case 7: return ARM_VSI7;
        default: return (ARM_VSI_Type*)0;
    }
}

static inline int ARM_VSI_Initialize(uint32_t channel) {
    if (channel > 7) return -1;
    return 0;
}

static inline void ARM_VSI_SetValue(uint32_t channel, uint32_t index, uint32_t value) {
    ARM_VSI_Type* vsi = arm_vsi_get_base(channel);
    if (vsi && index < 64) {
        vsi->Regs[index] = value;
    }
}

static inline uint32_t ARM_VSI_GetValue(uint32_t channel, uint32_t index) {
    ARM_VSI_Type* vsi = arm_vsi_get_base(channel);
    if (vsi && index < 64) {
        return vsi->Regs[index];
    }
    return 0;
}

static inline uint32_t ARM_VSI_GetData(uint32_t channel, void* data, uint32_t size) {
    ARM_VSI_Type* vsi = arm_vsi_get_base(channel);
    if (vsi) {
        vsi->DMA.Address = (uint32_t)(uintptr_t)data;
        vsi->DMA.BlockSize = size;
        vsi->DMA.BlockNum = 1;
        vsi->DMA.Control = ARM_VSI_DMA_Enable_Msk | ARM_VSI_DMA_Direction_P2M;
        while(vsi->DMA.Control & ARM_VSI_DMA_Enable_Msk); // Wait for completion
        return size;
    }
    return 0;
}
